codeunit 50003 "ARC Sales Line - Price" implements "Line With Price"
{
    var
    ...
    begin
        ...
        if (SalesLine.Type = SalesLine.Type::Item) and (SalesLine.Quantity <> 0) then begin
            PriceCalculationBuffer.Quantity := Abs(TotalQuantityOnAllOrders());
            SalesLine."DNK Pricing Quantity" := PriceCalculationBuffer.Quantity;
    ...
    local procedure TotalQuantityOnAllOrders(): Decimal
    var
    ...
    begin
        TotalQuantity := 0;
        if (SalesLine."Type" = "Sales Line Type"::Item) and (SalesLine.Quantity <> 0) then begin
            SalesLine2.Reset();
            SalesLine2.SetRange("Document Type", SalesLine."Document Type");
            SalesLine2.SetRange("Sell-to Customer No.", SalesLine."Sell-to Customer No.");
            SalesLine2.SetRange("Type", SalesLine."Type");
            SalesLine2.SetRange("Shipment Date", CalcDate('<-1Y>', Today), Today);
            if SalesLine2.FindSet(false) then
                repeat
                    TotalQuantity += (SalesLine2."Quantity" - SalesLine2."Quantity Shipped");
                until SalesLine2.Next() = 0;
            SalesLine2.SetFilter("Document Type", 'Return Order|Credit Memo');
            if SalesLine2.FindSet() then
                repeat
                    TotalQuantity -= (SalesLine2."Quantity" - SalesLine2."Return Qty. Received");
                until SalesLine2.Next() = 0;
            SalesShipmentLine.Reset();
            SalesShipmentLine.SetRange("Requested Delivery Date", CalcDate('<-1Y>', Today), Today);
            ...
            if SalesShipmentLine.FindSet(false) then
                repeat
                    TotalQuantity += SalesShipmentLine."Quantity";
